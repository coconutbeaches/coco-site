...
export default function RoomGalleryPreview({ label, coverUrl, images }: RoomGalleryPreviewProps) {
  const gallery = useMemo(() => {
    const unique = Array.from(new Set([coverUrl, ...images].filter(Boolean)));
    return unique;
  }, [coverUrl, images]);
  ...
  return (
    <>
      ...
      {open && (
        <div className="room-gallery-lightbox" role="dialog" aria-modal="true" aria-label={`${label} photo gallery`} onClick={() => setOpen(false)}>
